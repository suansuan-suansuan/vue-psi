...
export default {
    ...
    methods: {
        ...
        getData() {
            var vm =this;
            vm.axios.get("http://localhost:8089/cypsi/sys/getAllUser")
            .then(res => {
        ...
        handleDelete(row) {
            ...
            this.$confirm("确定要删除吗？", "提示", {
                ...
                .then(() => {
                   var vm=this;
                    vm.axios.delete("http://localhost:8089/cypsi/sys/delUser/"+row.uid)
                    .then(res =>{
                        if(res.data.success){
                            this.$notify({
                            title: '成功',
        ...
        saveEdit() {
            ...
            vm.axios.put("http://localhost:8089/cypsi/sys/updateUser",vm.form)
            .then(res => {
                if(res.data.success){
                    this.$notify({
                        title: '成功',
        ...
        saveAddUser(){
            ...
            vm.axios.post("http://localhost:8089/cypsi/sys/addUserInfo",vm.form)
            .then(res => {
                if(res.data.success){
                    this.$notify({
                        title: '成功',
        ...
        generateUcode(){
          ...
          this.form.ucode=year +month +date+"00"+(this.tableData.length+1);
        }
    }
 }
</script>
